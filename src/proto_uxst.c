...
static int uxst_bind_listener(struct listener *listener, char *errmsg, int errlen)
{
 ...
 fd_ready:
	 ...
	 */
	if (!ext && path[0] &&
	    (((listener->rx.ux.uid != -1 || listener->rx.ux.gid != -1) &&
	      (chown(tempname, listener->rx.ux.uid, listener->rx.ux.gid) == -1)) ||
	     (listener->rx.ux.mode != 0 && chmod(tempname, listener->rx.ux.mode) == -1))) {
		err |= ERR_FATAL | ERR_ALERT;

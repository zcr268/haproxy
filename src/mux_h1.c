...
#define H1C_F_DONT_READ      0x00100000 /* Don't read more data for now */
#define H1C_F_DONT_RCVBUF    0x00200000 /* Don't call xprt recv_buf for now (happens during splicing) */
#define H1C_F_IS_BACK        0x00400000 /* Set on outgoing connection */

#define H1C_F_CS_EMBRYIONIC  0x01000000 /* Set when a H1 stream with no conn-stream is attached to the connection */
#define H1C_F_CS_ATTACHED    0x02000000 /* Set when a H1 stream with a conn-stream is attached to the connection */
#define H1C_F_CS_ALIVE       (H1C_F_CS_IDLE|H1C_F_CS_EMBRYIONIC|H1C_F_CS_ATTACHED)
/*
...
static void h1_refresh_timeout(struct h1c *h1c)
{
	if (h1c->task) {
		...
		} else if ((!h1c->h1s && !(h1c->flags & H1C_F_IS_BACK)) || b_data(&h1c->obuf)) {
...
static struct conn_stream *h1s_new_cs(struct h1s *h1s, struct buffer *input)
{
	...
	h1s->h1c->flags = (h1s->h1c->flags & ~H1C_F_CS_EMBRYIONIC) | H1C_F_CS_ATTACHED;
...
static struct h1s *h1s_new(struct h1c *h1c)
{
	...
	h1c->flags = (h1c->flags & ~(H1C_F_CS_IDLE|H1C_F_WAIT_NEXT_REQ)) | H1C_F_CS_EMBRYIONIC;
...
static struct h1s *h1c_bck_stream_new(struct h1c *h1c, struct conn_stream *cs, struct session *sess)
{
	...
	h1c->flags = (h1c->flags & ~H1C_F_CS_EMBRYIONIC) | H1C_F_CS_ATTACHED;
...
static void h1s_destroy(struct h1s *h1s)
{
	if (h1s) {
		...
		h1c->flags &= ~(H1C_F_DONT_READ|H1C_F_DONT_RCVBUF|H1C_F_CS_EMBRYIONIC|H1C_F_CS_ATTACHED|
...
static void h1_set_req_tunnel_mode(struct h1s *h1s)
{
	...
	if (!(h1s->h1c->flags & H1C_F_IS_BACK)) {
		...
		if (h1s->res.state < H1_MSG_DONE) {
			h1s->h1c->flags |= H1C_F_DONT_READ;
			TRACE_STATE("Disable read on h1c", H1_EV_RX_DATA|H1_EV_H1C_BLK, h1s->h1c->conn, h1s);
		}
	}
	else if (h1s->h1c->flags & H1C_F_DONT_READ) {
		h1s->h1c->flags &= ~H1C_F_DONT_READ;
		tasklet_wakeup(h1s->h1c->wait_event.tasklet);
		TRACE_STATE("Enable read on h1c", H1_EV_RX_DATA|H1_EV_H1C_BLK|H1_EV_H1C_WAKE, h1s->h1c->conn, h1s);
	}
}
	...
	do {
		...
		if (h1m->state <= H1_MSG_LAST_LF) {
			...
			if (!(h1m->flags & H1_MF_RESP) && h1s->status == 101)
				...
				h1c->flags |= H1C_F_DONT_READ;
				TRACE_STATE("Disable read on h1c", H1_EV_RX_DATA|H1_EV_H1C_BLK, h1c->conn, h1s);
				break;
			}
			else
				break;
		...
		switch (h1m->state) {
			case H1_MSG_RQBEFORE:
				...
				if (h1c->flags & H1C_F_DONT_READ) {
					h1c->flags &= ~H1C_F_DONT_READ;
					h1c->conn->xprt->subscribe(h1c->conn, h1c->conn->xprt_ctx, SUB_RETRY_RECV, &h1c->wait_event);
					TRACE_STATE("Enable read on h1c", H1_EV_TX_DATA|H1_EV_H1C_BLK|H1_EV_H1C_WAKE, h1c->conn, h1s);
				}
				break;
...
static int h1_process(struct h1c * h1c)
{
	...
	if ((h1c->flags & H1C_F_DONT_RCVBUF) && !h1s_data_pending(h1s)) {
...
static size_t h1_rcv_buf(struct conn_stream *cs, struct buffer *buf, size_t count, int flags)
{
	...
	if (flags & CO_RFL_BUF_FLUSH) {
		if (h1m->state == H1_MSG_TUNNEL || (h1m->state == H1_MSG_DATA && h1m->curr_len)) {
			h1c->flags |= H1C_F_DONT_RCVBUF;
			TRACE_STATE("Block xprt rcv_buf to flush stream's buffer", H1_EV_STRM_RECV, h1c->conn, h1s);
		}
	}
	else {
...
static int h1_rcv_pipe(struct conn_stream *cs, struct pipe *pipe, unsigned int count)
{
	...
	if (!(h1c->flags & H1C_F_DONT_RCVBUF)) {
		h1c->flags |= H1C_F_DONT_RCVBUF;

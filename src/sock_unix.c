 ...
 */
int sock_unix_bind_receiver(struct receiver *rx, void (*handler)(int fd), void *owner, long tm, char **errmsg)
{
 ...
 fd_ready:
	 ...
	 */
	if (!ext && path[0] &&
	    (((rx->ux.uid != -1 || rx->ux.gid != -1) &&
	      (chown(tempname, rx->ux.uid, rx->ux.gid) == -1)) ||
	     (rx->ux.mode != 0 && chmod(tempname, rx->ux.mode) == -1))) {
		err |= ERR_FATAL | ERR_ALERT;

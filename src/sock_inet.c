 ...
 */
int sock_inet_bind_receiver(struct receiver *rx, void (*handler)(int fd), void *owner, long tm, char **errmsg)
{
	int fd, err, ext;
	...
	if (!ext) {
		fd = my_socketat(rx->netns, rx->proto->sock_family, rx->proto->sock_type, rx->proto->sock_prot);
		if (fd == -1) {
	...
	if (!ext && (rx->options & RX_O_FOREIGN)) {
	...
	if (!ext && rx->interface) {
		if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE,
			       rx->interface, strlen(rx->interface) + 1) == -1) {

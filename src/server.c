                            ...
                            struct proxy *curproxy, struct server *newsrv, char **err)
{
	...
	sk = str2sa_range(args[*cur_arg + 1], NULL, &port_low, &port_high, &errmsg, NULL, NULL, PA_O_RESOLVE | PA_O_SOURCE);
	if (!sk) {
	...
	while (*(args[*cur_arg])) {
		if (!strcmp(args[*cur_arg], "usesrc")) {  /* address to use outside */
			...
			if (!strcmp(args[*cur_arg + 1], "client")) {
				...
				/* 'sk' is statically allocated (no need to be freed). */
				sk = str2sa_range(args[*cur_arg + 1], NULL, &port1, &port2, &errmsg, NULL, NULL, PA_O_RESOLVE | PA_O_SOURCE);
				if (!sk) {
					ha_alert("'%s %s' : %s\n", args[*cur_arg], args[*cur_arg + 1], errmsg);
					goto err;
				}
                            ...
                            struct proxy *curproxy, struct server *newsrv, char **err)
{
	...
	/* 'sk' is statically allocated (no need to be freed). */
	sk = str2sa_range(args[*cur_arg + 1], NULL, &port_low, &port_high, &errmsg, NULL, NULL, PA_O_RESOLVE | PA_O_SERVER);
	if (!sk) {
		memprintf(err, "'%s %s' : %s\n", args[*cur_arg], args[*cur_arg + 1], errmsg);
		goto err;
	}
                 ...
                 struct proxy *defproxy, int parse_addr, int in_peers_section, int initial_resolve)
{
	...
	if (!strcmp(args[0], "server")         ||
		...
		if (!defsrv) {
			...
			sk = str2sa_range(args[cur_arg], &port, &port1, &port2, &errmsg, NULL, &fqdn, PA_O_SERVER | (initial_resolve ? PA_O_RESOLVE : 0));
			if (!sk) {

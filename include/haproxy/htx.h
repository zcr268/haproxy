...
/* Returns 1 if <blk> is the last block inside the HTX message <htx>, excluding
 * all unsued blocks. Otherwise, it returns 0. If 1 is returned, this means that
 * there is only <blk> and eventually some unused ones in <htx>.
 */
static inline int htx_is_last_blk(const struct htx *htx,
				  const struct htx_blk *blk)
{
